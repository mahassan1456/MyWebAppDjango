...
def stats(request):
    ...
    # NBA_TEAMS = ['Atlanta Hawks', 'Boston Celtics', 'Brooklyn Nets', 'Charlotte Hornets', 'Chicago Bulls', 'Cleveland Cavaliers', 'Dallas Mavericks', 'Denver Nuggets', 'Detroit Pistons', 'Golden State Warriors', 'Houston Rockets', 
    # 'Indiana Pacers', 'LA Clippers', 'LA Lakers', 'Memphis Grizzlies', 'Miami Heat', 'Milwaukee Bucks', 'Minnesota Timberwolves', 'New Orleans Hornets', 'New York Knicks', 'Oklahoma City Thunder', 'Orlando Magic', 
    ...
    if request.method == 'POST':
        ...
        # connection = sqlite3.connect(database='/Users/nefarioussmalls/Documents/Da Portfolio/NBA DB SQlite3/basketball.sqlite',uri=True)
        ...
        if r == "ALL":
            ...
            query = pd.read_sql_query(q, connection)
            # else:
            #     query = pd.read_sql_query(f"SELECT * from Game where GAME_DATE{r}'{date}'", connection)
        ...
        updated_html = "{{% extends 'polls/base.html' %}} \n {{% block content %}} <button id=\"button1\" onclick=\"Download()\" data-url=\"{URL}\" style='color:whitesmoke; background-color:rgb(63, 167, 150); border-radius:10px;'> <a style='text-decoration:none;' href=\"{{% url 'polls:download' \"{q}\" %}}\"> CSV </a> </button> \n {query} \n {script} \n {{% endblock %}}".format(q=q,query=query,URL=URL,script=script)
...
def download(request, query):
    ...
    # response["Content-Type"] = 'application/vnd.ms-excel' this works as well
    response["Content-Type"] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    response['Content-Disposition'] = 'attachment; filename={r}.xlsx'.format(r=raw_path)
    return response
    # in_memory_file = IO() # create an in memory file object that has no data but just the representation
    # path = r"/Users/nefarioussmalls/MyWebApp/mysite/" + raw_path
    # excel_writer2 = pd.ExcelWriter(r'/Users/nefarioussmalls/MyWebApp/mysite/nbagame', engine='xlsxwriter')
    # x = html_df.to_excel(path, index=False)
    # excel_writer2.save()
    # with open(path, 'rb') as file:
    #     response = HttpResponse(file.read(), content_type="application/ms-excel")
    #     response['Content-Disposition'] = 'attachment; filename={}_Report'.format(raw_path)
    #     messages.success(request, f"Your File Has been Downloaded on Local File Sytem at {PATH}")
    #     return response

    # print(x)
    # excel_writer = pd.ExcelWriter(in_memory_file, options={"in_memory": True}, engine='xlsxwriter') #create an excel writer object with the in_memory file as the source. If necessary, you can also use a 'path name'
    # html_df.to_excel(excel_writer=excel_writer, sheet_name="nbag",index=False) # use the dataframe to write to this in memory excel object holding your file
    # in_memory_file.seek(0) # now that you have wrote the file the buffer us at the very end so its needs to reset before generating the reponse object.
    # response =  HttpResponse(in_memory_file.read(), content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') # generate the response object with the in_memory exdel data with the right content_type so the response contains the right semantics so that it can be identified as an xlsx
    # response['Content-Disposition'] = 'attachment; filename=NBAGAME.xlsx' 
    # date = r"{}".format(date)
    # html_df.to_excel(excel_writer=raw_path, index=False)
    # csv = html_df.to_csv(f"./{date}", encoding='utf-8')
    # response = HttpResponse(content_type='application/vnd.ms-excel')
    # response['Content-Disposition'] = 'attachment; filename={}'.format(raw_path)
    messages.success(request, f"Your File Has been Downloaded on Local File Sytem at {PATH}")

    return response

def excel_write(df, path):
